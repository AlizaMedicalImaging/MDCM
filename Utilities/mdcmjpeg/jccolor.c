...
typedef struct
{
  ...
  IJG_INT * rgb_ycc_tab; /* => table for RGB to YCbCr conversion */
} my_color_converter;
...
#define CBCR_OFFSET ((IJG_INT)CENTERJSAMPLE << SCALEBITS)
...
rgb_ycc_start(j_compress_ptr cinfo)
{
  ...
  cconvert->rgb_ycc_tab = rgb_ycc_tab =
    (IJG_INT *)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE, (TABLE_SIZE * SIZEOF(IJG_INT)));
...
rgb_ycc_convert(j_compress_ptr cinfo, JSAMPARRAY input_buf, JSAMPIMAGE output_buf, JDIMENSION output_row, int num_rows)
{
  my_cconvert_ptr     cconvert = (my_cconvert_ptr)cinfo->cconvert;
  register int        r, g, b;
  register IJG_INT *  ctab = cconvert->rgb_ycc_tab;
