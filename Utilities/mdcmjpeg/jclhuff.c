...
typedef struct
{
  IJG_INT put_buffer; /* current bit-accumulation buffer */
...
start_pass_huff(j_compress_ptr cinfo, boolean gather_statistics)
{
  ...
  for (ci = 0; ci < cinfo->comps_in_scan; ci++)
  {
    ...
    else
    {
      ...
      jpeg_make_c_derived_tbl(cinfo, TRUE, dctbl, &entropy->derived_tbls[dctbl]);
    }
  }
...
/* Emit some bits; return TRUE if successful, FALSE if must suspend */
{
  ...
  register IJG_INT put_buffer = (IJG_INT)code;
                 ...
                 JDIMENSION     nMCU)
{
  ...
  for (ptrn = 0; ptrn < entropy->num_input_ptrs; ptrn++)
  {
    ...
    entropy->input_ptr[ptrn] = diff_buf[ci][MCU_row_num + yoffset] + (MCU_col_num * MCU_width);
  }
...
jinit_lhuff_encoder(j_compress_ptr cinfo)
{
  ...
  entropy =
    (lhuff_entropy_ptr)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE, SIZEOF(lhuff_entropy_encoder));
