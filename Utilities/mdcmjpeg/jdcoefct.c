...
decompress_onepass(j_decompress_ptr cinfo, JSAMPIMAGE output_buf)
{
  ...
  for (yoffset = coef->MCU_vert_offset; yoffset < coef->MCU_rows_per_iMCU_row; yoffset++)
  {
    for (MCU_col_num = coef->MCU_ctr; MCU_col_num <= last_MCU_col; MCU_col_num++)
    {
      ...
      jzero_far((void FAR *)coef->MCU_buffer[0], (size_t)cinfo->data_units_in_MCU * SIZEOF(JBLOCK));
      ...
      for (ci = 0; ci < cinfo->comps_in_scan; ci++)
      {
        ...
        output_ptr = output_buf[compptr->component_index] + yoffset * compptr->codec_data_unit;
...
jinit_d_coef_controller(j_decompress_ptr cinfo, boolean need_full_buffer)
{
  ...
  coef = (d_coef_ptr)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE, SIZEOF(d_coef_controller));
...
#  endif
  ...
  else
  {
    ...
    buffer =
      (JBLOCKROW)(*cinfo->mem->alloc_large)((j_common_ptr)cinfo, JPOOL_IMAGE, D_MAX_DATA_UNITS_IN_MCU * SIZEOF(JBLOCK));
