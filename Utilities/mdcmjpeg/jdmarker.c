...
/* Process a SOFn marker */
{
  ...
  TRACEMS4(
    cinfo, 1, JTRC_SOF, cinfo->unread_marker, (int)cinfo->image_width, (int)cinfo->image_height, cinfo->num_components);
...
LOCAL(void)
examine_app0(j_decompress_ptr cinfo, JOCTET FAR * data, unsigned int datalen, IJG_INT remaining)
 ...
 */
{
      ...
      GETJOCTET(data[2]) == 0x49 && GETJOCTET(data[3]) == 0x46 && GETJOCTET(data[4]) == 0)
  {
    ...
    if (totallen != ((IJG_INT)GETJOCTET(data[12]) * (IJG_INT)GETJOCTET(data[13]) * (IJG_INT)3))
      TRACEMS1(cinfo, 1, JTRC_JFIF_BADTHUMBNAILSIZE, (int)totallen);
  }
  else if (datalen >= 6 && GETJOCTET(data[0]) == 0x4A && GETJOCTET(data[1]) == 0x46 && GETJOCTET(data[2]) == 0x58 &&
...
LOCAL(void)
examine_app14(j_decompress_ptr cinfo, JOCTET FAR * data, unsigned int datalen, IJG_INT remaining)
