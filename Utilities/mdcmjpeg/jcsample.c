...
fullsize_downsample(j_compress_ptr cinfo, jpeg_component_info * compptr, JSAMPARRAY input_data, JSAMPARRAY output_data)
{
  ...
  /* Edge-expand */
  expand_right_edge(
    output_data, cinfo->max_v_samp_factor, cinfo->image_width, compptr->width_in_data_units * cinfo->data_unit);
}
...
h2v2_downsample(j_compress_ptr cinfo, jpeg_component_info * compptr, JSAMPARRAY input_data, JSAMPARRAY output_data)
{
  ...
  for (outrow = 0; outrow < compptr->v_samp_factor; outrow++)
  {
    ...
    {
      *outptr++ = (JSAMPLE)(
        (GETJSAMPLE(*inptr0) + GETJSAMPLE(inptr0[1]) + GETJSAMPLE(*inptr1) + GETJSAMPLE(inptr1[1]) + bias) >> 2);
      bias ^= 3; /* 1=>2, 2=>1 */
      inptr0 += 2;
      inptr1 += 2;
    }
                       ...
                       JSAMPARRAY            output_data)
{
  ...
  for (outrow = 0; outrow < compptr->v_samp_factor; outrow++)
  {
    ...
    neighsum = GETJSAMPLE(*above_ptr) + GETJSAMPLE(above_ptr[1]) + GETJSAMPLE(*below_ptr) + GETJSAMPLE(below_ptr[1]) +
    ...
    *outptr++ = (JSAMPLE)((membersum + 32768) >> 16);
    ...
    for (colctr = output_cols - 2; colctr > 0; colctr--)
    {
      ...
      neighsum +=
        GETJSAMPLE(above_ptr[-1]) + GETJSAMPLE(above_ptr[2]) + GETJSAMPLE(below_ptr[-1]) + GETJSAMPLE(below_ptr[2]);
