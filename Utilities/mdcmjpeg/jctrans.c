...
LOCAL(void) transencode_codec JPP((j_compress_ptr cinfo, jvirt_barray_ptr * coef_arrays));
...
transencode_coef_controller(j_compress_ptr cinfo, jvirt_barray_ptr * coef_arrays)
{
  ...
  coef = (c_coef_ptr)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE, SIZEOF(c_coef_controller));
  ...
  buffer =
    (JBLOCKROW)(*cinfo->mem->alloc_large)((j_common_ptr)cinfo, JPOOL_IMAGE, C_MAX_DATA_UNITS_IN_MCU * SIZEOF(JBLOCK));
...
LOCAL(void)
transencode_codec(j_compress_ptr cinfo, jvirt_barray_ptr * coef_arrays)
{
