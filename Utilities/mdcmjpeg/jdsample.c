...
typedef struct
{
   ...
   */
  IJG_UCHAR h_expand[MAX_COMPONENTS];
  IJG_UCHAR v_expand[MAX_COMPONENTS];
} my_upsampler;
             ...
             JDIMENSION       out_rows_avail)
{
  ...
  if (upsample->next_row_out >= cinfo->max_v_samp_factor)
  {
    for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components; ci++, compptr++)
    {
       ...
       */
      (*upsample->methods[ci])(
        cinfo, compptr, input_buf[ci] + (*in_row_group_ctr * upsample->rowgroup_height[ci]), upsample->color_buf + ci);
    }
...
#else
  register IJG_INT thiscolsum, lastcolsum, nextcolsum;
#endif
  register JDIMENSION colctr;
  int                 inrow, outrow, v;
...
jinit_upsampler(j_decompress_ptr cinfo)
{
  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components; ci++, compptr++)
  {
    ...
    else if ((h_out_group % h_in_group) == 0 && (v_out_group % v_in_group) == 0)
    {
      ...
      upsample->h_expand[ci] = (IJG_UCHAR)(h_out_group / h_in_group);
      upsample->v_expand[ci] = (IJG_UCHAR)(v_out_group / v_in_group);
    }
    else
