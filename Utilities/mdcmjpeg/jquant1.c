...
static const IJG_UCHAR base_dither_matrix[ODITHER_SIZE][ODITHER_SIZE] = {
...
/* The output values must fall in 0..MAXJSAMPLE in increasing order */
{
   ...
   */
  return (int)(((IJG_INT)j * MAXJSAMPLE + maxj / 2) / maxj);
}
...
make_odither_array(j_decompress_ptr cinfo, int ncolors)
{
   ...
   */
  den = 2 * ODITHER_CELLS * ((IJG_INT)(ncolors - 1));
  for (j = 0; j < ODITHER_SIZE; j++)
  {
    for (k = 0; k < ODITHER_SIZE; k++)
    {
      num = ((IJG_INT)(ODITHER_CELLS - 1 - 2 * ((int)base_dither_matrix[j][k]))) * MAXJSAMPLE;
