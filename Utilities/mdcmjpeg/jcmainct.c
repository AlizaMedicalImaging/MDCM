  ...
  while (mainPtr->cur_iMCU_row < cinfo->total_iMCU_rows)
  {
    ...
    if (mainPtr->rowgroup_ctr < data_unit)
      (*cinfo->prep->pre_process_data)(
        cinfo, input_buf, in_row_ctr, in_rows_avail, mainPtr->buffer, &mainPtr->rowgroup_ctr, (JDIMENSION)data_unit);
...
process_data_buffer_main(j_compress_ptr cinfo, JSAMPARRAY input_buf, JDIMENSION * in_row_ctr, JDIMENSION in_rows_avail)
{
  ...
  while (mainPtr->cur_iMCU_row < cinfo->total_iMCU_rows)
  {
    ...
    if (mainPtr->rowgroup_ctr == 0)
    {
      ...
      {
        mainPtr->buffer[ci] =
          (*cinfo->mem->access_virt_sarray)((j_common_ptr)cinfo,
                                            mainPtr->whole_image[ci],
...
jinit_c_main_controller(j_compress_ptr cinfo, boolean need_full_buffer)
{
  ...
  mainPtr = (my_main_ptr)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE, SIZEOF(my_main_controller));
  ...
  else
  {
    ...
    for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components; ci++, compptr++)
    {
      mainPtr->buffer[ci] = (*cinfo->mem->alloc_sarray)((j_common_ptr)cinfo,
                                                        JPOOL_IMAGE,
                                                        compptr->width_in_data_units * data_unit,
                                                        (JDIMENSION)(compptr->v_samp_factor * data_unit));
    }
