...
METHODDEF(void)
simple_downscale(j_compress_ptr cinfo, JSAMPROW input_buf, JSAMPROW output_buf, JDIMENSION width)
{
  ...
  for (xindex = 0; xindex < width; xindex++)
    output_buf[xindex] = (JSAMPLE)RIGHT_SHIFT(GETJSAMPLE(input_buf[xindex]), cinfo->Al);
}
