...
bool IPPSorter::Sort(std::vector<std::string> const & filenames)
{
  ...
  if(!b)
  {
    mdcmDebugMacro("Scanner failed");
    return false;
  }
  Scanner::ValuesType gantry = scanner.GetValues(tgantry);
  if(gantry.size() > 1)
  {
    mdcmDebugMacro("More than one Gantry/Detector Tilt");
    return false;
  }
  if(gantry.size() == 1)
  {
    ...
    if(tilt != 0.0)
    {
      mdcmDebugMacro("Gantry/Detector Tilt is not 0");
      return false;
    }
  }
  ...
  if(DirCosTolerance == 0.)
  {
    if(iops.size() != 1)
    {
      mdcmDebugMacro("More than one IOP (or no IOP): " << iops.size());
  ...
  {
    mdcmWarningMacro("Odd number of Frame Of Reference UID (continuing with caution): " << fsize);
  }
  else
  {
    mdcmErrorMacro("Sorry your setup with Frame Of Reference UID does not make any sense: " << fsize);
    return false;
  }
  ...
  if(it->first != tiop)
  {
    ...
    mdcmDebugMacro("No iop in first file ");
    return false;
  }
  ...
  if(!dircos)
  {
    ...
    mdcmDebugMacro("Empty iop in first file ");
    return false;
  }
      ...
      if(iskey)
      {
        ...
        if(value)
        {
          if(DirCosTolerance != 0.)
          {
            ...
            {
              if(value2)
                mdcmWarningMacro(filename << " cant read IOP: " << value2);
              return false;
            }
            ...
            {
              mdcmWarningMacro(filename << " Problem with DirCosTolerance: ");
              // Cant print cd since 0.9999 is printed as 1... may confuse user
              return false;
            }
          }
          //mdcmDebugMacro(filename << " has " << ipp << " = " << value);
          ...
          if(sorted.find(dist) != sorted.end())
          {
            ...
            {
              mdcmWarningMacro("dropping file " << filename << " since Z position: " << dist << " already found");
              continue;
            }
            mdcmWarningMacro("dist: " << dist << " for " << filename <<
              " already found in " << sorted.find(dist)->second);
            return false;
          }
          sorted.insert(SortedFilenames::value_type(dist,filename));
        }
        else
        {
          mdcmDebugMacro("File: " << filename << " has no Tag" << tipp << ". Skipping.");
        }
      }
      else
      {
        mdcmDebugMacro("File: " << filename << " could not be read. Skipping.");
      }
    }
  }
  assert(!sorted.empty());
  {
    ...
    if(sorted.size() > 1)
    {
      ...
      for(; it2 != sorted.end(); ++it2)
      {
        ...
        {
          mdcmDebugMacro("ZTolerance test failed. You need to decrease ZTolerance.");
          spacingisgood = false;
        }
